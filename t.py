...
def digits_of_pi(n):
    ...
    M = 2 * int(3. * n / logn / logn / logn)
    N = 1 + int((n + 15.) * math.log(10.) / (1. + math.log(2. * M)))
    N += N % 2
    m_max = M * N + N
    ...
    for k in range(N):
        ...
        s = mul_mod(s, pow_mod(10, n - N))
        s = mul_mod(s, 4)
        x += (2 * (k % 2) - 1) * float(s) / float(m)
    x = x - math.floor(x)
    return x
...
def PDC(n):
    ...
    pow10 = 1.e9
    y = x * pow10
    while pow10 > 10 and (y - math.floor(y) < 0.05 or y - math.floor(y) > 0.95):
        pow10 /= 10.0
        y = x * pow10
    return str(math.floor(y))

def trace_join(prev: str, new: str):
    for s in range(len(new)):
        pos = prev.find(new[:len(new) - s], -10)
        if pos != -1:
            return prev[:pos] + new
    return prev + new

def double_decimal_stream(prefix_pi: str):
    out = []
    carry = 0
    for ch in prefix_pi:
        if ch == '.':
...
def print_tau_stream():
    PAUSE_MIN_TIME = 0.05

    prefix_pi = "3.141592653589793238462643383279502884197169399375105"
    tau_prefix, carry = double_decimal_stream(prefix_pi)
    ...
    try:
        while True:
            dpi_ch = next_pi_digit()
            dpi = ord(dpi_ch) - 48
            t = 2 * dpi + carry
            out_digit = t % 10
            carry = t // 10
            print(chr(48 + out_digit), end='', flush=True)
